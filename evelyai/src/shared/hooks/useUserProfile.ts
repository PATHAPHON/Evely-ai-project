...
const KEY_PREFIX = 'evelyai:';
const LEGACY_PREFIXES = ['tranly:', 'tarnly:'];
...
function getStoredItem(key: string): string | null {
  ...
  for (const legacyPrefix of LEGACY_PREFIXES) {
    const legacy = safeLocalStorage.getItem(legacyPrefix + key);
    if (legacy !== null) {
      // Migrate legacy key
      safeLocalStorage.setItem(KEY_PREFIX + key, legacy);
      safeLocalStorage.removeItem(legacyPrefix + key);
      return legacy;
    }
  }
  return null;
